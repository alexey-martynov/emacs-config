...
(defun avm-get-configuration-directories ()
  ...
  (let (dirs)
    (when (file-exists-p "~/.emacs.d/site-lisp")
      (cl-labels ((directory-lister (path)
                    (mapc #'(lambda (file)
                              (when (file-directory-p file)
                                (setf dirs (cons file dirs))
                                (directory-lister file)))
                          (directory-files path t "[^.].*" t))))
        (directory-lister "~/.emacs.d/site-lisp")
  ...
  (let ((generated-autoload-file "~/.emacs.d/site-lisp/loaddefs"))
    (when (not (file-exists-p generated-autoload-file))
      (with-current-buffer (find-file-noselect generated-autoload-file)
        (insert ";; Automatically generated by site-start.el\n") ;; create the file with non-zero size to appease autoload
        (insert "(require 'compile)\n")
        (save-buffer)))
    (let ((dirs (avm-get-configuration-directories)))
      (when dirs
        (mapc #'(lambda (dir) (message "Extracting autoloads from %s" dir)) dirs)
        (apply  #'update-directory-autoloads dirs)))))

(defun recompile-all-configuration ()
  (let ((dirs (avm-get-configuration-directories)))
    ;; Special handling for SLIME via Quicklisp SLIME Helper
    (when (locate-library "~/quicklisp/slime-helper")
      (load "~/quicklisp/slime-helper")
      (push (quicklisp-slime-helper-slime-directory) dirs))
    (mapc #'(lambda (path)
